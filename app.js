...
document.addEventListener('DOMContentLoaded', function() {
    ...
    menuItems.forEach(item => {
        item.addEventListener('click', function(e) {
            ...
            // If it has a genre attribute, handle genre filtering
            if (genre && !link.href.includes('about')) {
                e.preventDefault();
...
function filterAnime(query) {
    ...
    if (query === '') {
        // If no query, show all and restore genre filter
        allMovieItems.forEach(item => item.style.display = 'block');
        filterByGenre(currentGenre);
        return;
    }
    ...
    // Filter items based on exact title match first, then partial matches
    allMovieItems.forEach(item => {
        ...
        const genres = item.getAttribute('onclick') ? item.getAttribute('onclick').toLowerCase() : '';
        
        // Exact title match gets priority
        const exactMatch = title === query;
        // Partial match in title
        const titleMatch = title.includes(query);
        // Match in description
        const descMatch = desc.includes(query);
        
        if (exactMatch || titleMatch || descMatch) {
            item.style.display = 'block';
            item.closest('.movie-list-container').style.display = 'block';
            hasAnyResults = true;
        } else {
    ...
    // Clean up: hide containers with no visible items
    containers.forEach(container => {
        const visibleItems = container.querySelectorAll('.movie-list-item[style="display: block;"]');
...
function openAnimeDetail(title, image, rating, year, description, episodes, genre) {
    ...
    // Initialize comment form and star rating
    updateCommentFormVisibility();
    setTimeout(() => {
        initializeStarRating();
    }, 100);
...
function postComment() {
    if (!isLoggedIn) {
        showNotification('Please sign in to post a review!');
        openLoginModal();
        return;
    }
